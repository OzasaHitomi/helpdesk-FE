...
import { useGetTicketsHandler } from './hooks/handlers/useGetTicketsHandler'

// Container: 各hookを呼び出して値を集め、Presentational（見た目）に橋渡しするだけの層
// ここにロジックは書かず、「誰から何を受け取って、誰にそのまま渡すか」だけに専念する
export const RootContainer = () => {
  const { data: meData } = useMeQuery()
  // 一覧の詰め替え・並び替えはuseGetTicketsHandler側が担当する（Containerはロジックを持たない）
  const { data: getTicketsData } = useGetTicketsHandler()
  ...
  return (
    <RootPresentational
      data={{
        ...
        tickets: getTicketsData.tickets,
      }}
