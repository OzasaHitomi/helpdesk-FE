import { useUnpublishTicketHandler } from '../useUnpublishTicketHandler'
import { customRenderHook } from '@/tests/helpers/customRenderHook'
import { describe, it, expect, vi, afterEach } from 'vitest'
import { act } from '@testing-library/react'

// mutateAsync/toaster.createのスパイを先に定義し、モジュールをまるごと差し替える
// (mutateAsyncの中身=通信処理はuseUnpublishTicketMutation.test.tsが担保するのでここではモックする)
...
vi.mock('../../mutations/useUnpublishTicketMutation', () => ({
...
describe('useUnpublishTicketHandler', () => {
  afterEach(() => {
    vi.clearAllMocks()
    mockIsPending.current = false
  })
  ...
  describe('非公開処理中の状態', () => {
    it('非公開処理中の場合、uiState.isSubmittingがtrueになること', () => {
      mockIsPending.current = true
      const { result } = customRenderHook(() => useUnpublishTicketHandler(1))

      expect(result.current.uiState.isSubmitting).toBe(true)
    })

    it('非公開処理中でない場合、uiState.isSubmittingがfalseになること', () => {
      const { result } = customRenderHook(() => useUnpublishTicketHandler(1))
  ...
  // ── onClick(非公開)の挙動 ──────────────────────────────────────────
  describe('非公開ボタンの操作', () => {
    it('非公開に成功した場合、mutateAsyncが呼ばれ、成功トーストが出ること', async () => {
    ...
    it('BEが422 BUSINESS_ERRORを返す場合、detailの文言のエラートーストが出ること', async () => {
      mockMutateAsync.mockRejectedValueOnce({
        isAxiosError: true,
        response: {
          data: { detail: '既に非公開設定です', type: 'BUSINESS_ERROR' },
        },
      })
      const { result } = customRenderHook(() => useUnpublishTicketHandler(1))
    ...
    it('BEが403等でdetail(文字列)を返す場合、その文言のエラートーストが出ること', async () => {
      mockMutateAsync.mockRejectedValueOnce({
        isAxiosError: true,
        response: {
          data: { detail: '質問者、サポート担当、または管理者のみ公開設定を変更できます' },
        },
      })
      const { result } = customRenderHook(() => useUnpublishTicketHandler(1))
      ...
      expect(mockToasterCreate).toHaveBeenCalledWith({
        type: 'error',
        title: '質問者、サポート担当、または管理者のみ公開設定を変更できます',
      })
    })
