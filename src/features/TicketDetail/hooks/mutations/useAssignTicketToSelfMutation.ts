...
import { assignTicketToSelf } from '@/services/internal/backend/v1/tickets'
...
export const useAssignTicketToSelfMutation = (ticketId: number) => {
  const queryClient = useQueryClient()

  return useMutation({
    mutationFn: () => assignTicketToSelf(ticketId),
    ...
    onSuccess: () => {
      ...
      // 担当者割り当て時にBE側で対応履歴（systemコメント）が追加されるため、そちらも再取得させる
      void queryClient.invalidateQueries({
