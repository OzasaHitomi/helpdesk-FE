...
// (mutateAsyncの中身=通信処理はuseActivateAccountMutation.test.tsが担保するのでここではモックする)
...
vi.mock('../../mutations/useActivateAccountMutation', () => ({
...
describe('useActivateAccountHandler', () => {
  ...
  describe('正常系', () => {
    ...
    it('利用再開に成功した場合、mutateAsyncが正しい引数で呼ばれ、成功トーストが出ること', async () => {
  ...
  // ── 異常系 ────────────────────────────────────────────────────────────────
  describe('異常系', () => {
    it('BEが422等でdetail(文字列)を返す場合、その文言のエラートーストが出ること', async () => {
      mockMutateAsync.mockRejectedValueOnce({
        isAxiosError: true,
        response: { data: { detail: '既に有効なユーザーです' } },
      })
      const { result } = customRenderHook(() => useActivateAccountHandler())
